...
class CtdetLoss(torch.nn.Module):
    def __init__(self,hm_weight,wh_weight,offset_weight,angle_weight):
    ...
    def forward(self, outputs, batch):
        hm_loss, wh_loss, off_loss , cls_loss= 0, 0, 0, 0
        for s in range(1):
            output = outputs[s]
            output['hm'] = _sigmoid(output['hm'])
            output['cls'] = _sigmoid(output['cls'])
            hm_loss += self.crit(output['hm'], batch['hm'])
            cls_loss += self.crit(output['cls'], batch['cls'])
        ...
        loss = self.hm_weight * hm_loss + self.wh_weight * wh_loss \
                + self.offset_weight * off_loss + self.angle_weight * cls_loss
